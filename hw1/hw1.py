# -*- coding: utf-8 -*-
"""Copy of hw1_regression.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1mQVA3UVgxR0LOPX187TlzpThA9XXcdw8
...
# on linux:
# data = pd.read_ csv(' /home/aistudio/data/data27964/train.csv', encoding = 'big5' ) 
file_a = sys.argv[1]
file_b = sys.argv[2]
file_c = sys.argv[3]

data = pd.read_csv(file_a, encoding = 'big5')
...
for month in range(12):
    ...
    for day in range(20):
        sample[:, day * 24 : (day + 1) * 24] = raw_data[18 * (20 * month + day) + 9, :]
    month_data[month] = sample
...
for t in range(iter_time):
    ...
    gradient = np.dot(x.transpose(), np.dot(x, w) - y) / len(y)
...
testdata = pd.read_csv(file_b, header = None, encoding = 'big5')
...
# testdata = pd.read_csv('./test.csv', header = None, encoding = 'big5')
test_data = testdata.iloc[:, 2:]
# test_data = testdata.iloc[:, 6:] # 5-9 hr
test_data[test_data == 'NR'] = 0
test_data = test_data.to_numpy()
...
import csv
with open(file_c, mode='w', newline='') as submit_file:
